"""Ticket suggestion strategies — the "for fun" heuristic lines.

⚠️ These rest on the gambler's fallacy: past draws do not influence future
ones, so NONE of them can improve your odds. They're transparent, reproducible
ways to pick numbers "for fun" and to compare alongside the position-based
models — nothing more.
...
from analyze import days_since_last, load_draws, recent_frequency
...
def suggest(product_name: str, strategy: str = "balanced",
    ...
    lines = [_weighted_sample(weights, product.main_count, rng) for _ in range(tickets)]
    return {"product": product.label, "strategy": strategy,
            "window": window, "tickets": lines}


def suggest_all(product_name: str, tickets: int = 1, seed: int | None = None) -> dict:
    """One line per strategy. Each strategy gets an offset seed so a fixed
    seed doesn't make every strategy identical."""
    out = {}
